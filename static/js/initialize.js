...
exports.aceInitialized = function(hook, context){
    console.log(clientVars.ep_profile_modal)
    if(clientVars.profile_json == null){
        ...
        var modal = $("#ep_profile_askmodal_script").tmpl(clientVars.ep_profile_modal);
        $("body").append(modal)
    }else{
        $("#pad_title").append("<div id='ep-profile-button'><img id='ep-profile-image'  src='"+clientVars.ep_profile_modal.profile_image_url+"' /></div>")
        var modal = $("#ep_profile_modal_script").tmpl(clientVars.ep_profile_modal);
        $("body").append(modal)
    }

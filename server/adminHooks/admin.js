...
exports.registerRoute = (hookName, args, cb) => {
  args.app.get('/admin/ep_profile_modal', (req, res) => {
    ...
    res.send(eejs.require('ep_profile_modal/templates/admin/admin.html', renderArgs));
  });
  args.app.get('/admin/ep_profile_modal_analytics', (req, res) => {
    const renderArgs = {
      errors: [],
    };
    res.send(eejs.require('ep_profile_modal/templates/admin/analytics.html', renderArgs));
  });
  return [];
};
...
exports.eejsBlock_adminMenu = (hookName, args, cb) => {
  ...
  const urlPrefix = hasAdminUrlPrefix
    ? 'admin/' : hasTwoDirDown ? '../../' : hasOneDirDown ? '../' : '';
...
exports.socketio = (hookName, args, cb) => {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('load-analytics', async (data) => {
      const padUsers = await db.get(`ep_profile_modal_contributed_${data.pad}`) || [];

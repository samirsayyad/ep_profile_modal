    ...
    if (settings.settingsDomain && settings.settingsEmailSmtp && settings.settingsEmailPort && settings.settingsEmailUser && settings.settingsEmailPassword) {
      ...
      if (userEmail) {
        if (!user.verified) {
          ...
          const link = `https://${settings.settingsDomain}/p/emailConfirmation/${Buffer.from(userId).toString('base64')}/${Buffer.from(padId).toString('base64')}/${Buffer.from(confirmCode).toString('base64')}`;
          const html = `<p><b>Hello ${userName}! </b></p>

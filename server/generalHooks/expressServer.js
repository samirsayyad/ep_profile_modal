...
const defaultImg =
  '/static/plugins/ep_profile_modal/static/dist/img/user-blue.png';
const defaultImgUserOff =
...
exports.expressConfigure = (hookName, context) => {
  ...
  // ///////       getContributors         //////////////////////
  context.app.get(
      '/static/:padId/pluginfw/ep_profile_modal/getContributors/:page',
      async (req, res, next) => {
        const padId = req.params.padId;
        const page = parseInt(req.params.page) || 1;
        let contributorList =
        (await db.get(`ep_profile_modal_contributedList_${padId}`)) || [];
        const outputList = [];
  ...
  context.app.get(
      '/static/:padId/pluginfw/ep_profile_modal/getUserInfo/:userId',
      async (req, res, next) => {
        const padId = req.params.padId;
  ...
  context.app.get(
      '/static/emailConfirmation/:userId/:padId/:confirmCode',
      async (req, res, next) => {
        ...
        if (user.confirmationCode === confirmCode) {
          if (user.image !== '' && user.image !== 'reset' && user.image) {
            user.image =
            (await moveImageToAccount(userId, padId, user.email, user.image)) ||
            user.image;
          }
          ...
          if (emailUser) {
            user.pads = emailUser.pads || [];
          } // store pads of verified users
          if (user.pads) {
        ...
        // logged in
          if (user.image && user.image !== 'reset') {
            ...
            if (profileJson !== 'User not found') {
              httpsUrl = gravatar.url(user.email, {
                protocol: 'https',
                s: '200',
              });
            } else {
  ...
  context.app.get(
      ...
      async (req, res, next) => {
        ...
        settings.settingsEmailSmtp &&
        settings.settingsEmailPort &&
        settings.settingsEmailUser &&
        settings.settingsEmailPassword
          ...
          const user =
          (await db.get(`ep_profile_modal:${userId}_${padId}`)) || {};
          let userEmail = '';
          ...
          if (userEmail) {
            if (!user.verified) {
              ...
              // eslint-disable-next-line no-eval
                ? eval(settings.settingsHtmlBodyTemplate)

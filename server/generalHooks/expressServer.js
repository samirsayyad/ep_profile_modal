...
const validation = (variable) => {
  if (!variable || variable === 'null' || variable === '') { return false; } else { return true; }
};
...
exports.expressConfigure = (hookName, context) => {
  ...
  context.app.get('/static/:padId/pluginfw/ep_profile_modal/getContributors/:page', async (req, res, next) => {
    ...
    if (contributorList.length) {
      contributorList = contributorList.sort((a, b) => b.lastSeenTimestamp - a.lastSeenTimestamp);
      const slicedArray = contributorList.splice(offset, getContributorsLimit);
      await Promise.all(slicedArray.map(async (row) => {
        const user = await db.get(`ep_profile_modal:${row.userId}_${padId}`) || {};
        const userImage = `/static/getUserProfileImage/${row.userId}/${padId}?t=${new Date().getUTCDay()}`;
        outputList.push({
          userId: row.userId,
          email: user.email || '',
          ...
          lastSeenDate: ((user.lastSeenDate === today) ? 'today' : (user.lastSeenDate === yesterday) ? 'yesterday' : user.lastSeenDate) || '',
          lastSeenTimestamp: user.lastSeenTimestamp || 0,
        });
      })
      );
    }
  ...
  context.app.get('/static/emailConfirmation/:userId/:padId/:confirmCode', async (req, res, next) => {
    ...
    if (user.confirmationCode === confirmCode) {
      ...
      let verifiedEmail = await db.get(`ep_profile_modal_verified_email_${padId}`);
      if (verifiedEmail) {
        if (verifiedEmail.indexOf(user.email) === -1) {
      ...
      await db.set(`ep_profile_modal_verified_email_${padId}`, verifiedEmail);
      // gathering verified email of pads

      // gathering verified emails
      let verifiedEmails = await db.get('ep_profile_modal_verified_emails');
      if (verifiedEmails) {
        if (verifiedEmails.indexOf(user.email) === -1) {
      ...
      db.set('ep_profile_modal_verified_emails', verifiedEmails);
      // gathering verified emails
    ...
    if (settings.settingsDomain && settings.settingsEmailSmtp && settings.settingsEmailPort && settings.settingsEmailUser && settings.settingsEmailPassword) {
      ...
      if (userEmail) {
        if (!user.verified) {
          ...
          const link = `https://${settings.settingsDomain}/static/emailConfirmation/${Buffer.from(userId).toString('base64')}/${Buffer.from(padId).toString('base64')}/${Buffer.from(confirmCode).toString('base64')}`;
          const html = (settings.settingsHtmlBodyTemplate) ? eval(settings.settingsHtmlBodyTemplate) : `<p><b>Hello ${userName}! </b></p>
  ...
  context.app.post('/static/:padId/pluginfw/ep_profile_modal/upload/:userId', async (req, res, next) => {
    ...
    busboy.on('file', async (fieldname, file, filename, encoding, mimetype) => {
      ...
      file.on('end', async () => {
        ...
        const selctedWidth = 128;
        const selectedHeight = 128;
        let newFile;
        try {
          // resize and reshape
